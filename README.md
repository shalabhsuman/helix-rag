...
![helix-rag pipeline overview](docs/social_preview.png)
...
| Vector search | No, math only | Cosine similarity in Qdrant |
| BM25 search | No, math only | Term frequency scoring |
...
```bash
uv pip install -e ".[dev,agent,eval]"       # adds RAGAS + DeepEval (Phase 5)
uv pip install -e ".[dev,agent,eval,ui]"    # adds Gradio (Phase 7)
```
...
There are five scripts. Each has a single responsibility.
...
| `scripts/run_agent.py` | For a conversational session | Starts an interactive chat loop with the agent. Maintains conversation history across turns. |
...
`ingest.py` builds the database. `query.py` and `run_agent.py` use it. The last two are for measuring quality.
...
|---|---|---|
| Every push to any branch | All unit tests | Free (no API calls) |
| Every pull request | All unit tests | Free |
| Merge to main | All unit tests + full eval suite | Small API cost |
...
pytest requires writing test functions. ruff and mypy only need to be configured once in `pyproject.toml` and inspect the code automatically from that point on.

To run all three locally before pushing:
...
| Eval | `eval.yml` | Merge to main + manual | Full RAGAS + DeepEval evaluation against golden dataset | ~10 min |
...
To see the index status in Qdrant: open http://localhost:6333/dashboard, click `helix_rag`, and look at the **Status** field. Green means the index is built and ready.
